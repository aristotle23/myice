...
class User
{
    ...
    public  function setContact($name, $phone, $address){
        $pcheck = $this->db->getOne("select phone from usercontacts where phone = ?", array($phone));

        if($pcheck != null){
            return false;
        }
        $id = $this->db->executeGetId("insert into usercontacts (user_id, name, phone, address) values (?,?,?,?)",array($this->userId,$name,$phone,$address));
        if($id){
            return true;
        }
        return false;
    ...
    public function getContact(){
        $contacts = $this->db->getAll("select * from usercontacts where user_id = ?",array($this->userId));
        if(count($contacts) < 1){
            return false;
        }
        return $contacts;
